...
public class CubePresentationScript : MonoBehaviour 
{
	...
	void Start() 
	{
		...
		_gestureManager = GetComponent<GestureManager>().instance;
	}
	...
	void Update() 
	{
		...
		if(!isSpinning)
		{
			...
			if(slideChangeWithGestures && _gestureManager)
			{
				if(_gestureManager.IsSwipeLeft())
					RotateLeft();
				else if(_gestureManager.IsSwipeRight())
					RotateRight();
				else if(_gestureManager.IsSwipeUp())
					RotateUp();
			}
		}
		else
