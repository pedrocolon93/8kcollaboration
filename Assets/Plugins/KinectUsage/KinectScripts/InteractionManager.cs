...
public class InteractionManager : MonoBehaviour 
{
    ...
    public Texture gripHandTextureLeft;
    public Texture gripHandTextureRight;
	[Tooltip("Hand-cursor texture for the hand-release state.")]
    public Texture releaseHandTextureLeft;
    public Texture releaseHandTextureRight;
	[Tooltip("Hand-cursor texture for the non-tracked state.")]
    public Texture normalHandTextureLeft;
    public Texture normalHandTextureRight;
	...
	public bool IsInteractionInitiated()
	{
		...
		if(kinectManager && kinectManager.IsInitialized())
		{
			...
			if(playerUserID != 0)
			{
				...
				if(isLeftHandInteracting)
				{
					...
					if(controlMouseCursor && 
					   (!showHandCursor || (!gripHandTextureRight && !releaseHandTextureRight && !normalHandTextureRight)))
					{
...
//				}
		...
		if(showHandCursor)
		{
			...
			{
				if(lastLeftHandEvent == HandEventType.Grip)
					texture = gripHandTextureLeft;
				else if(lastLeftHandEvent == HandEventType.Release)
					texture = releaseHandTextureLeft;
			}
			else if(isRightHandPrimary)
			{
				if(lastRightHandEvent == HandEventType.Grip)
					texture = gripHandTextureRight;
				else if(lastRightHandEvent == HandEventType.Release)
					texture = releaseHandTextureRight;
			}
			
			if(texture == null)
			{
                if (isLeftHandPrimary)
                    texture = normalHandTextureLeft;
                else
                    texture = normalHandTextureRight;
			}
