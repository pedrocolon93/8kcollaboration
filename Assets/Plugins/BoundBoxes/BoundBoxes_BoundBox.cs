...
public class BoundBoxes_BoundBox : MonoBehaviour {
	...
	private List<BoundBoxes_drawLines> cameralines;
    ...
    public void updateCameraLines()
    {
        cameralines = new List<BoundBoxes_drawLines>();

        for (int i = 0; i < GameObject.Find("CameraContainer").transform.childCount; i++)
        {
            if (GameObject.Find("CameraContainer").transform.GetChild(i).gameObject.activeSelf)
            {
                
                string name = "Player" + i + "Camera";

                Camera foundcam = GameObject.Find("CameraContainer").transform.GetChild(i).gameObject.GetComponent<Camera>();
                cameralines.Add(foundcam.GetComponent<BoundBoxes_drawLines>());
            }
	...
	public void init() {
		...
		setPoints();
		setLines();
		foreach(BoundBoxes_drawLines cameraline in cameralines)
			cameraline.setOutlines(lines,lineColor);
	}
	...
	void Update() {
        ...
        for(int i = 0 ; i<cameralines.Count;i++)
            cameralines[i].setOutlines(lines,lineColor);
	}
    void LateUpdate()
    {
